...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/compute_A_Arrhenius_field.log", mode="w"),  # Overwrite log file each run
...
def main(): 
    ...
    logging.debug("Calling fw.omega_dot_T")
    Model_field = fw.omega_dot_T(
        rhoM, TM, Y1M, Y2M, Y3M, Y4M, Y5M, TEMP, kappa, epsilon, W_k, nu_k, h_f
    )
    Model_field = Model_field + 1e-12 #np.finfo(float).eps  # Avoid division by zero
    A_Arr = HRR_M / Model_field

    wfu.write_to_file(write_path, "pre_exponential_factor_field", A_Arr)
    wfu.write_to_file(write_path, "HRR_by_A", Model_field)
