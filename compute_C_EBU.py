...
Y_O2_B = np.array(0.0423, dtype=np.float64)
#Y_O2_U = np.array(0.222606, dtype=np.float64)
Y_O2_U = np.array(0.2300, dtype=np.float64)
...
def main():
    ...
    for blk in blks:
        ...
        temp = np.ones_like(rho)
        kappa = np.ones_like(rho)
        epsilon = np.ones_like(rho)
